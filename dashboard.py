...
def render_scanner() -> Panel:
    ...
    content.add_column("Gemini", justify="right", width=6, style=ACCENT)
    ...
    if not state.latest_markets:
        ...
        return Panel(content, title="[bold]MARKET SCANNER[/bold]  ·  Gemini Confidence vs Market Odds", border_style="bright_green", box=box.ROUNDED)
...
def render_trades() -> Panel:
    ...
    table.add_column("Gemini", justify="right", width=6)
